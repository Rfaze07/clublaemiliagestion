...
exports.getLista = async (req, res) => {
    try {
        res.render('recuerdosPaneros/views/index', {
            pagename: "Recuerdos Pañeros",
...
exports.alta = async (req, res) => {
    try {
        ...
        return res.json({ status: true, icon: 'success', title: 'Éxito', type: 'success', text: 'Recuerdo Pañero dado de alta correctamente' })
    } catch (error) {
        console.log(error)
...
exports.modificar = async (req, res) => {
    try {
        ...
        return res.json({ status: true, icon: 'success', title: 'Éxito', type: 'success', text: 'Recuerdo Pañero modificado correctamente' })
    } catch (error) {
        console.log(error)
        return res.json({ status: false, icon: 'error', title: 'Error', type: 'error', text: 'Error del servidor' })
    }
...
exports.eliminar = async (req, res) => {
    try {
        let id = req.body.id
        let recuerdoPanero = await model.getById(id)
        if (recuerdoPanero.status === 0) {
            return res.json({ status: false, icon: 'error', title: 'Error', type: 'error', text: 'Recuerdo Pañero no encontrado' })
        }
        // Eliminar imagen de Cloudinary si existe
        if (recuerdoPanero.length && recuerdoPanero[0].imagen_public_id) {
            await cloudinary.uploader.destroy(recuerdoPanero[0].imagen_public_id).catch(() => {})
        }
        let result = await model.delete(id)
        if (result.status === 0) {
            return res.json({ status: false, icon: 'error', title: 'Error', type: 'error', text: result.text })
        }
        return res.json({ status: true, icon: 'success', title:'Éxito', type: 'success', text: 'Recuerdo Pañero eliminado correctamente' })
    } 
    catch (error) {
        console.log(error)
        return res.json({ status: false, icon: 'error', title: 'Error', type: 'error', text: 'Error del servidor' })
