...
exports.getLista = async (req, res) => {
    try {
        res.render('equipos/views/index', {
            pagename: "Equipos",
...
const ValidarCampos = o => {
    return new Promise((resolve, reject) => {
        if(String(o.nombre).trim().length == 0) return res.json({ status: false, icon:'error', title: 'Error', text: 'Debe ingresar la descripción' })
        if(String(o.nombre).trim().length >= 40) return res.json({ status: false, icon:'error', title: 'Error', text: 'La descripcion supero la cantidad permitida' })
