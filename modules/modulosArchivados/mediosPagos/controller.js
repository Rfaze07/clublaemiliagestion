...
exports.getLista = async (req, res) => {
    try {
        res.render('mediosPagos/views/index', {
...
exports.getListaActivosAjaxSelect = async (req, res) => {
    try {
        const data = await model.getAllbyActivo(true)
        res.json({ status: true, data })
    } catch (error) {
        console.log(error)
        res.json({ status: false, type: "error", icon:'error', title: 'Error', text: 'Hubo un error al procesar la solicitud' })
    }
}
...
exports.postAlta = async (req, res) => {
    try {
        if((req.body.desc_corta).trim().length == 0) return res.json({ status: false, type: 'error', icon:'error', title: 'Error', text: 'Debe ingresar la descripción corta' })
        if((req.body.desc_corta).trim().length > 5) return res.json({ status: false, type: 'error', icon:'error', title: 'Error', text: 'La descripción corta superó la cantidad permitida (Max.: 5 caracteres)' })
        ...
        if((req.body.descripcion).trim().length > 100) return res.json({ status: false, type: 'error', icon:'error', title: 'Error', text: 'La descripción superó la cantidad permitida (Max.: 100 caracteres)' })
