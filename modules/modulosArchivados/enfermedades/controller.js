...
const mInasistencias = require('../inasistencias/model');
...
exports.getLista = async (req, res) => {
    try {
        res.render('enfermedades/views/index', {
...
exports.postAlta = async (req, res) => {
    try {
        ...
        await eventos.insertarEvento({ usuario: req.session.user.id, tabla: "Enfermedades", acc: "a", registro: req.body.id })
...
exports.postModificar = async (req, res) => {
    try {
        ...
        await eventos.insertarEvento({ usuario: req.session.user.id, tabla: "Enfermedades", acc: "m", registro: req.body.id })
...
exports.postEliminar = async (req, res) => {
    try {
        let inasistencias = await mInasistencias.getAllByEnfermedad(req.body.id);
        if(inasistencias.length > 0) return res.json({ status: false, type: "warning", icon: "warning", title: "Alerta", text: "No se puede eliminar la enfermedad, existen inasistencias asociadas" })
        
        let result = await model.delete(req.body.id)
        if(result.affectedRows == 0) return res.json({ status: false, type: "error", icon: "error", title: "Error", text: "Hubo un error al procesar la solicitud" })
        await eventos.insertarEvento({ usuario: req.session.user.id, tabla: "Enfermedades", acc: "b", registro: req.body.id })
        return res.json({ status: true, type: "success", icon: "success", title: "Éxito", text: "Solicitud procesada correctamente" })
    } catch (error) {
        console.log(error)
        return res.json({ status: false, type: "error", icon: "error", title: "Error", text: "Hubo un error al procesar la solicitud" })
