...
exports.getLista = async (req, res) => {
    try {
        res.render('sucursales/views/index', {
            pagename: "Sucursales",
...
const ValidarCampos = o => {
    return new Promise(async (resolve, reject) => {
        ...
        if(String(o.descCorta).trim().length > 5) resolve({ status: false, icon:'error', title: 'Error', text: 'La descripción corta superó la cantidad máxima de caracteres permitidos' })
        ...
        if(String(o.direccion).trim().length > 100) resolve({ status: false, icon:'error', title: 'Error', text: 'La dirección superó la cantidad máxima de caracteres permitidos' })
