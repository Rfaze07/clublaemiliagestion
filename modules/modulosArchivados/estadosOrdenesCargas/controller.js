...
exports.getLista = async (req, res) => {
    try {
        res.render('estadosOrdenesCargas/views/index', {
            pagename: "Estados de OC.",
...
exports.postEliminar = async (req, res) => {
    try {
        let ordenesCargas = await mOrdenesCargas.getAllByEstadosOrdenes(req.body.id)
        if(ordenesCargas.length) return res.json({ status: false, icon: "warning", title: "Alerta", text: "No se puede eliminar el registro seleccionado, tiene ordenes de carga asociadas" })
...
const ValidarCampos = o => {
    return new Promise(async (resolve, reject) => {
        ...
        if(String(o.descCorta).trim().length > 3) resolve({ status: false, icon:'error', title: 'Error', text: 'La descripción corta superó la cantidad máxima de caracteres permitidos. (Máx.: 4)' })
        resolve({ status: true })
    })
}