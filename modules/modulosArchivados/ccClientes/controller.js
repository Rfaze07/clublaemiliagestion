...
exports.getLista = async (req, res) => {
	try {
		res.render("ccClientes/views/lista", {
			pagename: "C/c Clientes",
...
exports.getListaAjax = async (req, res) => {
	try {
		...
		let paramsComprob = [req.body.hasta],
			queryComprob = `
				SELECT c.*, cl.razon_social AS clientetxt, cl.nro_documento AS cuit 
				FROM comprobantes c
				LEFT JOIN clientes cl ON cl.id = c.id_cliente_fk
				LEFT JOIN empresas e ON e.id = c.id_empresa_fk
				WHERE c.id_cliente_fk > 0 AND c.fecha_comprobante < ?
			`
		...
		if(req.body.cliente != 't'){
			queryComprob += ` AND c.id_cliente_fk = ?`
			paramsComprob.push(req.body.cliente)
		}
