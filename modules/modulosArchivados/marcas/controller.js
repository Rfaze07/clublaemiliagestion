...
const mMarcas = require('../marcas/model')
...
exports.postAlta = async (req, res) => {
    try {
        ...
        if(String(req.body.descripcion).trim().length > 100) return res.json({ status: false, type: 'error', icon:'error', title: 'Error', text: 'La descripcion supero la cantidad permitida.' })
        ...
        if(String(req.body.desc_corta).trim().length > 5) return res.json({ status: false, type: 'error', icon:'error', title: 'Error', text: 'La descripcion corta supero la cantidad permitida.' })
...
exports.postEliminar = async (req, res) => {
    try {
        let vehiculos =  await mVehiculos.getAllByMarca(req.body.id)
        if(vehiculos.length > 0) return res.json({ status: false, type: "error", icon: "error", title: "Error", text: "No se puede eliminar. Marca  vinculada  a un vehiculo" })
        let marcas =  await mMarcas.getAllByMarca(req.body.id)
        if(marcas.length > 0) return res.json({ status: false, type: "error", icon: "error", title: "Error", text: "No se puede eliminar. Marca  vinculada  a un repuesto" })
