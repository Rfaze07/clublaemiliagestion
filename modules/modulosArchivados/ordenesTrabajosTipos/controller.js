...
const mOT = require('../ordenesTrabajos/model')
...
exports.postEliminar = async (req, res) => {
    try {
        //CHEQUEO SI ESTA ASIGNADA A OT
        let ots = await mOT.getByTipoOT(req.body.id)
        if (ots.length > 0) return res.json({ status: false, icon: "error", title: "Error", text: "Tipo de OT asignada a Orden de Trabajo. No se puede eliminar" })
        
        let result = await model.delete(req.body.id)
        if(result.affectedRows == 0) return res.json({ status: false, icon: "error", title: "Error", text: "Hubo un error al procesar la solicitud" })           
        return res.json({ status: true, icon: "success", title: "Exito", text: "Solicitud procesada correctamente" })
