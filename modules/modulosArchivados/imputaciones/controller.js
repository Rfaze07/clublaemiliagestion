...
exports.getLista = async (req, res) => {
    try {
        res.render('imputaciones/views/lista', {
            pagename: "Imputaciones",
...
exports.getListaSelectAjax = async (req, res) => {
    try {
        ...
        if(!data.length) return res.json({ status: false, icon:"warning", title: "Alerta", text: "No existen imputaciones cargados" })
...
exports.postEliminar = async (req, res) => {
    try {
        //CHEQUEO NOVEDADES
        let novedades = await mNovedades.getByImputaciones(req.body.id)
        if(tareas.length > 0) return res.json({ status: false, icon: "error", title: "Error", text: "Esta imputacion se encuentra asignada a una o mas novedades, no se puede eliminar" })
        if(novedades.length > 0) return res.json({ status: false, icon: "error", title: "Error", text: "Esta imputacion se encuentra asignada a una o mas novedades, no se puede eliminar" })


        //CHEQUEO MOVIMIENTOS
        let movimientos = await mOrdenesTrabajos.getMovimientosByImputacion(req.body.id)
        if(movimientos.length > 0) return res.json({ status: false, icon: "error", title: "Error", text: "Esta imputacion se encuentra asignada a uno o mas movimientos, no se puede eliminar" })
